...
_Root cause connue (cf. #33) : `objet.dossierLegislatif.dossierRef` est_ null _dans l'export AN pour une fraction significative des scrutins — y compris des Scrutins décisifs bien réels sur des dossiers identifiables, pas seulement les votes de détail (amendement/article) où c'est attendu. Lacune de la donnée source, pas une absence réelle de lien : contournée au cas par cas par curation manuelle (`DOSSIER_REF_OVERRIDE` dans `src/spi/filesystem/scrutinRepository.ts`) après rapprochement de titre avec `Dossiers_Legislatifs.json.zip`, et surveillée à chaque mise à jour de `data/raw/an/` par le script de non-régression `npm run audit:dossiers` (`src/spi/filesystem/auditDossiersManquants.test.ts`), qui échoue si un dossierRef déjà connu (brut ou curé) a un Scrutin décisif sans fiche `content/dossiers/`. Les scrutins décisifs sans dossierRef résolu du tout (aucune curation existante) restent à trier manuellement, cf. #41._
_Avoid_: Texte, Proposition/Projet de loi (trop spécifique — un dossier peut aussi désigner une proposition de résolution)
...
_Cas limite observé_ : une motion de rejet préalable peut être déposée et adoptée par la majorité elle-même (pas l'opposition), comme manœuvre de procédure pour transmettre le texte au Sénat sans débat d'amendements plutôt que pour le tuer — le dossier poursuit alors sa navette normalement (cf. DLR5L17N50819, motion adoptée en 1re lecture suivie d'une CMP puis d'un vote sur l'ensemble). Se vérifie par la poursuite effective de la navette (un acte législatif postérieur existe), pas par l'intention déclarée du dépôt ; non détecté automatiquement, curé au cas par cas via `DOSSIER_REF_OVERRIDE`.
